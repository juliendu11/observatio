...
test.group('Inertia - alerts/update', (group) => {
  ...
  test('Should add new setting when is manager', async ({ client }) => {
  ...
  test('Should add new setting when user is a user is not manager and has correct permission', async ({
    client,
  }) => {
    const defaultOrg = await Organization.firstOrFail()
    defaultOrg.related('setting').create({})
  ...
  test('Should not add new setting with not allowed message when user is not manager and has not correct permission', async ({
    client,
  }) => {
    const defaultOrg = await Organization.firstOrFail()
    defaultOrg.related('setting').create({})
