...
export default class HlsToMp4Job implements JobHandlerContract<HlsToMp4Payload> {
  ...
  async handle(job: Job<HlsToMp4Payload>) {
    ...
    const diskFileService = await app.container.make(DiskFileService)
    ...
    const m3u8Content = await diskFileService.readFile(hlsStreamPath)
    const lastChunk =
      m3u8Content
        .split('\n')
    ...
    await diskFileService.createFolderIfNotExist(mp4Folfer)
